...
@export var book_size_supported: int = 0:
	set(value):
		book_size_supported = value
		process_lod(Engine.is_editor_hint())
...
func get_max_supported_length() -> int:
	return max(0, book_size_supported)
...
func _load_static():
	...
	if !Engine.is_editor_hint():
		books = $"/root/GlobalLibrary".get_book_range_from_distance(self.allocation.start, self.allocation.size, false)
	else:
		var reached_book_size = 0
		while reached_book_size < self.get_max_supported_length():
	...
	var possible_space = self.get_max_supported_length() * self.scale_for_spacing
	...
	if Engine.is_editor_hint():
		...
		var length_to_use_mm: float = float(get_max_supported_length()) * scale_for_spacing + extra_space_for_randomness_mm
...
func _unload_static():
	if self.debug_child_mesh != null:
		self.debug_child_mesh.queue_free()
	$PreviewMesh.set("scale", Vector3(get_max_supported_length() * 0.001 * scale_for_spacing, 1, 1))
	$PreviewMesh.set("position", Vector3(get_max_supported_length() * 0.001 * scale_for_spacing / 2 + random_selected_offset, 0.2/2, -0.15/2))
